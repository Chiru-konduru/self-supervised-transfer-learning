...
class YoloLoss(nn.Module):
    ...
    def find_best_iou_boxes(self, pred_box_list, box_target):
        ...
        for b in range(len(pred_box_list)):
            ...
            # diagonal of iou matrix 
            iou = iou.diag() # shape: (N, )
            ious[:, b] = iou
    ...
    def get_class_prediction_loss(self, classes_pred, classes_target, has_object_map):
        ...
        ### CODE ###
        # Your code here
        class_sum = has_object_map * (classes_pred - classes_target).pow(2).sum(dim=-1) 
        loss = class_sum.sum()
        return loss
    ...
    def get_no_object_loss(self, pred_boxes_list, has_object_map):
        ...
        for b in range(len(pred_boxes_list)):
            confidence = pred_boxes_list[b][:, :, :, 4][has_no_obj_map] 
            no_obj_loss += F.mse_loss(confidence, torch.zeros_like(confidence), reduction='sum')

        loss = no_obj_loss * self.l_noobj
        return loss
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        for i in range(len(pred_boxes_list)):
            # reshape the boxes and keep only the cells with objects
